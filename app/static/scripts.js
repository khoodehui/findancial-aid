...
//Thumbs Down button
$(".td-btn").click(function (event) {
  if ($(this).hasClass("td-btn-checked")) {
    $(this).removeClass("td-btn-checked").addClass("td-btn-unchecked");
    const url = "/background_process_remove_not_interested/" + event.target.id;
    $.getJSON(url, function (data) {
      //do nothing
    });
    return false;
  } else {
    $(this).removeClass("td-btn-unchecked").addClass("td-btn-checked");
    const url = "/background_process_not_interested/" + event.target.id;
    $.getJSON(url, function (data) {
      //do nothing
    });
    return false;
